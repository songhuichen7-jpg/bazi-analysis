...
def _ten_god_facts(chart: dict[str, Any]) -> list[str]:
    """Build an authoritative ten-god + 透藏 + 干合 fact sheet.

    The polisher LLM regularly mislabels ten-gods (e.g. calling 甲日丁火
    "食神" when it is actually 伤官) because the prompt only ships the
    main 用神 string, leaving 干支 ten-god identity and 藏-vs-透 status to
    the model's recall. Computing it here from paipan data and pinning
    it into the prompt closes the gap."""
    try:
        from paipan.shi_shen import get_shi_shen
    except Exception:  # noqa: BLE001 — keep polisher resilient if engine missing
        return []

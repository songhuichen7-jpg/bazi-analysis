...
test('LandingHome covers the editorial single-page narrative', () => {
  ...
  assert.match(source, /电影、音乐、天气、气味、书和一卦/);
...
test('LandingHome introduces movie, music, weather, perfume, book, and gua cards as aesthetic product previews', () => {
  const source = fs.readFileSync(new URL('../src/components/landing/LandingHome.jsx', import.meta.url), 'utf8');
  const css = fs.readFileSync(new URL('../src/styles/landing.css', import.meta.url), 'utf8');
  ...
  assert.match(source, /用一种天气形容我现在的状态/);
  assert.match(source, /雨后初雾/);
  assert.match(source, /weather-mist-dawn\.jpg/);
  assert.match(source, /用一种气味形容我这盘/);
  assert.match(source, /冷茶白花/);
  assert.match(source, /perfume-cold-tea\.jpg/);
  assert.match(source, /推荐一本书让我读懂自己/);
  assert.match(source, /夜读手记/);
  assert.match(source, /book-quiet-page\.jpg/);
  assert.match(source, /这件事现在要不要推进/);
  assert.match(source, /小亨，旅贞吉/);
  assert.match(source, /landing-play-card/);
  assert.match(source, /MediaCard/);
  assert.doesNotMatch(source, /真实卡片样子/);
  assert.doesNotMatch(source, /landing-real-card-stage/);

  assert.match(css, /\.landing-play-grid[\s\S]*grid-template-columns:\s*repeat\(6/);
  ...
  assert.match(css, /\.landing-play-gua/);

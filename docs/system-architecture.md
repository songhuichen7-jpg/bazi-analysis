...
│   │   ├── llm/             # OpenAI-compatible LLM client + streaming
...
DeepSeek API 走 OpenAI 兼容协议。同样是分层策略：
- 重型任务 (sections / verdicts / dayun-step / liunian / chat) → `chat_stream_with_fallback` (主模型优先, 首 delta 超时切 fallback)
...
| 模块 | 用途 |
|---|---|
| `loader.py` | runtime 优先加载 docs/bazi-analysis/SKILL.md + docs/bazi-analysis/conversation-guide.md，回退到 docs/skills；同时加载 shards/*.md |
...
# 后端 (FastAPI on :3101)
cd server
cp .env.example .env   # 填入 DEEPSEEK_API_KEY + DB_URL
